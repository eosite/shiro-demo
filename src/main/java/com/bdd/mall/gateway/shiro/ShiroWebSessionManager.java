...
public class ShiroWebSessionManager extends DefaultSessionManager implements WebSessionManager {
    ...
    private String getSessionIdHeaderValue(ServletRequest request, ServletResponse response) {
        if (!(request instanceof HttpServletRequest)) {
            ...
            if (StringUtils.isEmpty(sessionId)) {
                sessionId = httpRequest.getParameter(X_AUTH_TOKEN);
            }
    ...
    private Serializable getReferencedSessionId(ServletRequest request, ServletResponse response) {
        String id = getSessionIdHeaderValue(request, response);
    ...
    private void removeSessionIdHeader(HttpServletRequest request, HttpServletResponse response) {
        response.setHeader(X_AUTH_TOKEN, "deleteMe");
    }
    ...
    private void storeSessionId(Serializable currentId, HttpServletRequest ignored, HttpServletResponse response) {
        if (currentId == null) {
            ...
            response.setHeader(X_AUTH_TOKEN, idString);
